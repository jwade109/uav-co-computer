...
#include <mav_offb_control/rxtx.hpp>
...
void global_pos_get(const nav_msgs::Odometry::ConstPtr& msg){
    local_pos = *msg;
}
...
{
    ros::init(argc, argv, "offb_reader");
    ros::NodeHandle nh;
    ...
    ros::Subscriber global_pos_local = nh.subscribe<nav_msgs::Odometry>
           ("mavros/global_position/local", 25, global_pos_get);
  ...
  */
    ...
    while(ros::ok() && !current_state.connected){
        ROS_INFO_STREAM_THROTTLE(3, "Waiting!");
        ros::spinOnce();
        rate.sleep();
    }
    ...
    while(ros::ok()){
      ...
      //if (current_state.armed){
       /*ROS_INFO_STREAM("Snap!");

       rd = read(fd, &buf[0], sizeof(charbuf));
       ROS_INFO_STREAM("Snap1!");  // data from arduino
       ...
       ROS_INFO_STREAM("Snap2!");*/
        ...
        local_pos.pose.pose.orientation.w<<","<<local_pos.pose.pose.orientation.x<<","<<local_pos.pose.pose.orientation.y<<","
        <<local_pos.pose.pose.orientation.z<<","<<imu_data.orientation.w<<","<<imu_data.orientation.x<<","<<imu_data.orientation.y
        ...
        odom_data.pose.pose.position.y<<","<<odom_data.pose.pose.position.z<<","<<local_pos.pose.pose.position.x<<","<<
        local_pos.pose.pose.position.y<<","<<local_pos.pose.pose.position.z<<","<<alt_data.amsl<<","<<
        alt_data.relative<<","<<alt_data.local<<","<<"\n";

        ROS_INFO_STREAM("Snap3!");
